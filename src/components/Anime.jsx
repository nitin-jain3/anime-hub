...
const Anime = () => {
  ...
  const [animeSrc, setAnimeSrc] = useState(animeArr[0]);
  return <Stack direction={['column', 'row']} h={'90vh'}>
      <VStack w={'full'}>
        <video 
          src={animeSrc}
          controls
      ...
      >
        {
          animeArr.map((item, index) => (
            <Button variant={'ghost'} colorScheme={'purple'} onClick={() => setAnimeSrc(item)}>
            Episode {index}
            </Button>
          ))
        }
